<script lang="ts">
	...
	import { ChangeAvatar } from '$lib/wailsjs/go/main/App';
	let crop = { x: 0, y: 0 };
	let zoom = 1;
	...
	async function submitAvatar() {
		...
		const requestData = {
			...
			fileName: file.name,
			cropY: croppingElements.pixels.y,
			cropX: croppingElements.pixels.x,
			cropWidth: croppingElements.pixels.width,
			cropHeight: croppingElements.pixels.height,
			oldAvatar: old_avatar
		};

		if ($servers[`servers:${$page.params.serverId}`]) {
			requestData.serverId = `servers:${$page.params.serverId}`;
		}

		if ($friends) {
			requestData.friends = $friends.map((friend) => friend.id);
		}

		const response = await ChangeAvatar(JSON.stringify(requestData));
